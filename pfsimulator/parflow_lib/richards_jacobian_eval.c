                       ...
                       N_Vector pressure)
{
  ...
  if (*recompute)
  {
    ...
    */
    ...
    (ThisPFModule = richards_jacobian_eval,
     RichardsJacobianEval(pressure, old_pressure, &J, &JC, saturation, density,
                                                      ...
                                                      * full Jacobian */
{
   ...
   *-----------------------------------------------------------------------*/
  InlineNewVectorType(grid, 1, 1, vector_cell_centered, density_der);
  InlineNewVectorType(grid, 1, 1, vector_cell_centered, saturation_der);
...
/* Define grid for surface contribution */
  ...
  if (!public_xtra->initialized)
  {
    public_xtra->initialized = 1;
    ForSubgridI(is, GridSubgrids(grid))
    {
      ...
      public_xtra->flags = ctalloc(Flag, (nx*ny*nz));
      Flag *__flags = public_xtra->flags;

      SetInteriorFlags(i, j, k, gr_domain, r, ix, iy, iz, nx, ny, nz, __flags);
      SetBCFlags(ipatch, bc_struct,
      {
        SetBCFlag(DirichletBC, DirBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
        SetBCFlag(FluxBC, FluBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
        SetBCFlag(OverlandBC, OvlBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
        SetBCFlag(SeepageFaceBC, SeeBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
        SetBCFlag(OverlandKinematicBC, OvlKBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
        SetBCFlag(OverlandDiffusiveBC, OvlDBC, i, j, k, nx, ny, nz,
                  fdir, ival, bc_struct, ipatch, is, __flags);
      });
    }
  }

  Flag *_flags = public_xtra->flags;
  Flag _flag;
  ...
  ForSubgridI(is, GridSubgrids(grid))
  {
    ...
    /*              ddp[iv] * pp[iv] + sp[iv] * dp[iv]); */
    /* }); */
    OMP_BoxLoopI0(i, j, k, ix, iy, iz, nx, ny, nz,
    {
      im = SubmatrixEltIndex(J_sub, i, j, k);
      ipo = SubvectorEltIndex(po_sub, i, j, k);
      iv = SubvectorEltIndex(d_sub, i, j, k);
  ...
  */
  ...
  ForSubgridI(is, GridSubgrids(grid))
  {
    ...
    {
      /* _flag = GetFlag(_flags, i, j, k, nx, ny, nz); */
              ...
              else
              {
                ...
                if (diffusive == 0)
                {
                  ...
                  PFModuleInvokeType(OverlandFlowEvalInvoke, overlandflow_module,
                                     ...
                                     ke_der, kw_der, kn_der, ks_der, NULL, NULL, CALCDER));
                  */
                  OVERLAND_FLOW_MODULE(overlandflow_module, grid, is, bc_struct, ipatch,
                ...
                else
                {
                  ...
                  */
                  OVERLAND_DIFFUSIVE_MODULE(overland_flow_module_diff, grid, is, bc_struct,
...
//                  printf("Jac SF: CP=%f im=%d  \n", cp[im], im);
        ...
        case OverlandKinematicBC:
        {
          ...
          */
          OVERLAND_KINEMATIC_MODULE(overlandflow_module_kin, grid, is, bc_struct,
                                    ipatch, problem_data, pressure,
                                    ke_der, kw_der, kn_der, ks_der,
                                    NULL, NULL, NULL, NULL, NULL, NULL, CALCDER);
  ...
  PFModuleInvokeType(RichardsBCInternalInvoke, bc_internal, (problem, problem_data, NULL, J, time,
                                                             pressure, CALCDER));
  */
  RICHARDS_INTERNAL_MODULE(bc_internal, problem, problem_data, NULL, J, time, pressure, CALCDER);
 ...
 * the spinup process */
  ...
  public_xtra->initialized = 0;
  public_xtra->flags = NULL;
